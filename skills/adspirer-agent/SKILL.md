...
**When the user wants something to happen on a schedule, use Hermes cron.** Offer to create it with
the `cronjob` tool. Attach the relevant Adspirer skill and use the brand project's work directory so
the run can read `BRAND.md` and `STRATEGY.md`.
...
If the user wants a server-side alert that continues independently of their Hermes gateway, or wants
the result by **email**, Adspirer's `monitoring_and_reporting` router provides monitors, scheduled
briefs, and research jobs. Discovery on it is free: `{"action": "list_tools"}`.
